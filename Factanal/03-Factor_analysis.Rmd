...
all_melt <- melt(applygetRes,measure.vars = likert_variables1)
all_melt$Context <- all$Context[match(all_melt$Var1,all$Resp.ID)]

all_melt <- all_melt %>% separate(Var2,into=c("item","type"),sep="\\.",remove=FALSE)
ggplot(all_melt,aes(x=Var2,y=value)) + geom_bar(position = "stack",colour="black",stat="identity") + 
